...
class MultiqcModule(BaseMultiqcModule):
    def __init__(self):
        ...
        if config.kwargs.get("disable_mgikit_plugin", True):
            return None
        ...
        super(MultiqcModule, self).__init__(
            ...
            target="mgikit",
            anchor="mgikit",
            href="https://github.com/sagc-bioinformatics/mgi-demultiplexing-rust",
        ...
        if len(mgi_lane_statistics.keys()) > 0:
            self.add_section(
                ...
                plot=table.plot(mgi_lane_statistics, columns_headers, {"sortRows": False, 'col1_header': 'Run ID - Lane'}),
            )
        ...
        for f in sample_read_logs:
            ...
            for line in lines[1:]:
                ...
                if vals[0] in mgi_lane_sample_read_data[curr_label].keys():
                    log.warning(
                        "Same sample ({}) appears twice in the same log file!".format(
                            vals[0]
                        )
                    )
        ...
        if file_cnt > 0:
            log.info(
                "{} Read information log files (*.mgikit.info) were loaded!".format(
                    file_cnt
                )
            )
            ...
            if (
                not mgi_sample_reads_data_filtered
                or len(mgi_sample_reads_data_filtered.keys()) == 0
            ):
                log.info(
                    "No sample left after filteration!".format(config.analysis_dir)
                )
                raise UserWarning
            else:
                pconfig = {
                ...
                sample_read_plt = bargraph.plot(
                    mgi_sample_reads_data_filtered, cats, pconfig=pconfig
                )
                ...
                if not brief_report:
                    ...
                    for lane in sorted(mgi_lane_read_data.keys()):
                        ...
                        sample_read_plt = bargraph.plot(
                            mgi_lane_sample_read_data[lane], cats, pconfig=pconfig
                        )
                        ...
                        self.add_section(
                            ...
                            description="This plot shows the number of reads per sample for Lane: "
                            + lane,
                            helptext="This longer string (can be **markdown**) helps explain how to interpret the plot",
                            plot=sample_read_plt,
                        )
        ...
        undetermined_barcode_logs = self.find_log_files(
        ...
        if file_cnt > 0:
            log.info(
                "{} Undetermined barcode log files (*.mgikit.undetermined_barcode) were loaded!".format(
                    file_cnt
                )
            )
            ...
            lane_barcode_plt = bargraph.plot(
                sorted_dic, cat_lane, pconfig=pconfig
            )
        ...
        if file_cnt > 0:
            log.info(
                "{} Ambiguous barcode log files (*.mgikit.ambiguous_barcode) were loaded!".format(
                    file_cnt
                )
            )
            sorted_dic = OrderedDict(
            ...
            self.add_section(
                ...
                helptext="This longer string (can be **markdown**) helps explain how to interpret the plot",
                plot=table.plot(sorted_dic, columns_headers, {"sortRows": False, 'col1_header': 'Barcode'}),
            )
        #else:
