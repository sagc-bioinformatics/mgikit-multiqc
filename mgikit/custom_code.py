...
config.mgikit_plugin_version = get_distribution("multiqc_mgikit_plugin").version


# Add default config options for the things that are used in MultiQC_NGI
def mgikit_plugin_execution_start():
    ...
    if config.kwargs.get("disable_mgikit_plugin", True):
        return None
    log.info("Running mgikit MultiQC Plugin v{}".format(config.mgikit_plugin_version))
    ...
    config.module_order.append({"mgikit": {"module_tag": ["DNA", "RNA"]}})

    # Add to the search patterns used by modules
    if "mgikit/mgi_info" not in config.sp:
        config.update_dict(
            config.sp, {"mgikit/mgi_sample_reads": {"fn": "*.mgikit.info"}}
        )

    if "mgikit/mgi_undetermined_barcode" not in config.sp:
        config.update_dict(
            config.sp,
            {
    ...
    if "mgikit/mgi_ambiguous_barcode" not in config.sp:
        config.update_dict(
            config.sp,
            {"mgikit/mgi_ambiguous_barcode": {"fn": "*.mgikit.ambiguous_barcode"}},
        )


    if "mgikit/mgi_general_info" not in config.sp:
        config.update_dict(
            config.sp,
            {
